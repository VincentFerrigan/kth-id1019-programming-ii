...
\subsection*{The overall Work-flow} % \todo BYT NAMN?
...
The function, and its derivative (in its simplified form), and the calculation result are printed to the console
...
\begin{enumerate}
    \item
    \item \mintinline{elixir}{ast}: The mathematical expression to be differentiated (type `expr()`).
...
% /Todo include an exception
%## Examples
%
%#      iex> run({:sin, {:var, :x}}, :x, 0)
%#      # Output of the derivative, simplified form, and calculated value at x = 0
%"""
\begin{longlisting}
    \inputminted[
        label=Derivative.run/3,
